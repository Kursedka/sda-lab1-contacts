...
void expandArray(int additional) {
    ...
    if (newArray == NULL) { printf("Ошибка выделения памяти!\n"); return; }
    for (int i = 0; i < size; i++) newArray[i] = contacts[i];
    free(contacts);
    contacts = newArray;
...
void popBack() {
    if (size == 0) { printf("Список контактов пуст, удалять нечего.\n"); return; }
    size--;
...
void manualFree() {
    if (contacts == NULL || size == 0) { printf("Память уже освобождена или список пуст.\n"); return; }
    printf("Освободить всю память? (y/n): ");
    char ch;
    scanf(" %c", &ch);
    while (getchar() != '\n');
    if (ch == 'y' || ch == 'Y') clearArray();
    else printf("Операция отменена.\n");
}

void saveToFile() {
    FILE* f = fopen(SAVE_FILE, "w");
    if (f == NULL) { printf("Ошибка открытия файла для записи!\n"); return; }
    fprintf(f, "%d\n", size);
...
void loadFromFile() {
    ...
    if (f == NULL) { printf("Файл %s не найден. Начинаем с пустого списка.\n", SAVE_FILE); return; }
    int newSize;
    fscanf(f, "%d", &newSize);
    while (getchar() != '\n');
    if (newSize <= 0) { fclose(f); return; }
    clearArray();
...
void add_contact() {
    ...
    printf("Имя: "); scanf("%s", newContact.name); while (getchar() != '\n');
    printf("Телефон: "); scanf("%s", newContact.phone); while (getchar() != '\n');
    printf("Email: "); scanf("%s", newContact.email); while (getchar() != '\n');
    printf("Дата рождения (ДД.ММ.ГГГГ): "); scanf("%s", newContact.birthday); while (getchar() != '\n');
    printf("Категория (личные/работа/другое): "); scanf("%s", newContact.category); while (getchar() != '\n');
    pushBack(newContact);
    printf("Контакт успешно добавлен!\n");
}

void print_contacts() {
    if (size == 0) { printf("Список контактов пуст.\n"); return; }
    printf("\n========== ВСЕ КОНТАКТЫ ==========\n");
    for (int i = 0; i < size; i++) {
        printf("Контакт #%d\n", i + 1);
        printf("  Имя: %s\n", contacts[i].name);
...
void edit_contact() {
    if (size == 0) { printf("Нет контактов для редактирования.\n"); return; }
    int index;
    printf("Введите номер контакта для редактирования (1-%d): ", size);
    scanf("%d", &index); while (getchar() != '\n');
    if (index < 1 || index > size) { printf("Неверный номер!\n"); return; }
    index--;
    ...
    switch (choice) {
    case 1: printf("Новое имя: "); scanf("%s", contacts[index].name); while (getchar() != '\n'); break;
    case 2: printf("Новый телефон: "); scanf("%s", contacts[index].phone); while (getchar() != '\n'); break;
    case 3: printf("Новый email: "); scanf("%s", contacts[index].email); while (getchar() != '\n'); break;
    case 4: printf("Новая дата рождения (ДД.ММ.ГГГГ): "); scanf("%s", contacts[index].birthday); while (getchar() != '\n'); break;
    case 5: printf("Новая категория: "); scanf("%s", contacts[index].category); while (getchar() != '\n'); break;
...
void search_contacts() {
    ...
    if (choice == 1) {
        ...
        scanf("%s", search_name); while (getchar() != '\n');
        int found = 0;
        ...
        scanf("%s", search_cat); while (getchar() != '\n');
        int found = 0;
...
void sortContacts() {
    if (size < 2) { printf("Недостаточно контактов для сортировки.\n"); return; }
    int field;
    printf("\nВыберите поле для сортировки:\n");
    printf("1 - по имени\n2 - по телефону\n3 - по email\n4 - по дате рождения\n5 - по категории\n");
    printf("Ваш выбор: ");
    scanf("%d", &field); while (getchar() != '\n');
...
void insertAtIndex() {
    ...
    if (pos < 1 || pos > size + 1) { printf("Неверная позиция!\n"); return; }
    for (int i = size; i >= pos; i--) contacts[i] = contacts[i - 1];
    struct Contact newContact;
...
void deleteAtIndex() {
    ...
    scanf("%d", &pos); while (getchar() != '\n');
    if (pos < 1 || pos > size) { printf("Неверный номер!\n"); return; }
    for (int i = pos - 1; i < size - 1; i++) contacts[i] = contacts[i + 1];
    size--;
